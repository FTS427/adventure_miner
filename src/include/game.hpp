...
inline Player
login(std::filesystem::path player_data_path)
{
  ...
  if (load_status == 0) {
    ...
    for (scanf("Please enter your password again: %s", pl_passwd);
         pl.check_passwd(pl_passwd);
         scanf("Please enter your password again: %s", pl_passwd))
      ;
    return pl;
  } else {
    if (load_status == 1) {
      io::error("Can't load player's data!");
    } else if (load_status == 2) {
      io::error("Empty player's data file!");
    } else {
      io::error("Can't find " + pl.get_name() + "'s data!");
    }

    bool input_status =
      io::y_or_n("You are new, would you want to save your data?", false);
    if (input_status) {
